...
function extendFunctions(functionName) {
	var result = [],
		x;

	for (x = arguments.length - 1; x > 0; x--) {
		if (arguments[x][functionName] === null) {
			break;
		}

		if (!arguments[x][functionName]) {
			continue;
		}

		if (Object.prototype.toString.call(arguments[x][functionName]) === '[object Array]') {
			result = result.concat(arguments[x][functionName].reverse());
			arguments[x][functionName].reverse();
		}

		if (typeof arguments[x][functionName] === 'function') {
			result = result.concat(arguments[x][functionName]);
		}
	}
...
Executor.prototype.defaults = {
	extendRules: {
		prefilter: extendFunctions.bind(null, 'prefilter'),
		processResult: extendFunctions.bind(null, 'processResult')
	},
	transport: H.request,
	Deferred: H.Deferred,
	queue: H.queue,
