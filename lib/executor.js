...
Executor.prototype.addStageToQueue = function (taskQueue, stage, wiredArgs) {
	...
	if (typeof options[stage.name] === 'function') {
		taskQueue.push(options[stage.name].bind(options.ctx, arg));
	}

	if (Object.prototype.toString.call(options[stage.name]) === '[object Array]') {
		for (var x = 0; x < options[stage.name].length; x++) {
			taskQueue.push(options[stage.name][x].bind(options.ctx, arg));
		}
	}
};
...
Executor.prototype.smartExtend = function (optionsChain) {
	...
	return extend.apply(null , optionsChain);
};
