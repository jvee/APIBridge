...
function pluginIniter(executor, options) {
	var x, stage, stages, extendRule, extendRules, pluginDesc;
	...
	for (x = 0; x < plugins.length; x++) {
		...
		pluginDesc = plugins[x];

		if (typeof plugins[x] === 'function') {
			pluginDesc =  plugins[x](/* executor, apiBridge, helpers */);
		}

		stages = pluginDesc.stages || {};
		extendRules = pluginDesc.extendRules || {};
